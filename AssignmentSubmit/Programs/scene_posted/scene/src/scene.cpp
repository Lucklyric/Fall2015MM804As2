...
Color Scene::getPointLightColor(Ray *ray, PointLight *ptLight, double distance)// distance is the distance to the source
{
	...
	if (!ifOb)
	{
		// No block return the light color
		col = ptLight->col;
	}
    return(col);
}
...
geomObj* Scene::findFirstIntersection( Ray *ray, RTfloat *k,
	...
	{
		RTfloat check;
		// Check by the self intersect functions
                        ...
                        Vector3 normal)				// normal
{
    Ray rRay;
   // complete this function
	// Get the reflected vector and construct the ray
